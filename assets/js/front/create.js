...
$(document).ready(function(){
    ...
    $.getJSON( "data/medium.json", function( data ) {
        let html2 = ""
        for (let i = 0; i < 7; i++) {
            html2 += '<a target="_blank" href="'+data.data[i].link+'"class="list-group-item list-group-item-action flex-column align-items-start"><div class="d-flex w-100 justify-content-between"><h5 class="mb-1">'+data.data[i].name+'</h5><small>'+data.data[i].date+'</small></div><p class="mb-1">'+data.data[i].resume+'</p></a>'
        }
        $("#medium_article").html(html2)
    });

    $.getJSON( "blog/data/blog.json", function( data ) {
        let html2 = ""
        let count = 1
        for (let i = (data.data.length - 1); i >= 0 ; i--) {
            html2 += '<a target="_blank" href="blog/'+data.data[i].link+'"class="list-group-item list-group-item-action flex-column align-items-start"><div class="d-flex w-100 justify-content-between"><h5 class="mb-1">'+data.data[i].title+'</h5><small>'+data.data[i].date+'</small></div><p class="mb-1">'+data.data[i].sort_content+'</p></a>'
            if (count == 7) {
                break
            }
            count++
        }
        $("#blog_article").html(html2)
    });
});
